...
- **Deferred** — Tracked; active mitigation begins on a defined trigger (e.g., Azure cut-over)
...
| R-004 | Source repository hosted under personal GitHub account (`DanAyache`) rather than a GitHub organization; governance and ownership tied to an individual account | 2 | 4 | **8 Medium** | CODEOWNERS covers security-sensitive paths (verified per F-15 §4.A P-3); branch protection on `main`, required PR reviews, and personal-account MFA inferred per F-15 §4.B P-2/P-4 — screenshot evidence target 2026-06-30 | Evaluate migration to organization ownership or implement equivalent governance controls | Engineering Lead | Open | 2026-09-01 |
...
| R-006 | Tampering with GitHub Actions workflows to bypass security checks or inject malicious build steps | 2 | 4 | **8 Medium** | CODEOWNERS protects `.github/workflows/` (verified per F-15 §4.A P-3); branch protection on `main`, required PR reviews, and required status checks inferred per F-15 §4.B P-2 — screenshot evidence target 2026-06-30 | Pin third-party Actions to commit SHAs (not version tags) at next workflow update; audit `.github/` directory each quarterly review | Engineering Lead | In Progress | 2026-09-01 |
...
| R-016 | Azure migration control coverage — the documented control set was authored for Vercel/Supabase/GitHub; Azure access, secret, logging, change-management, and backup controls are not yet designed or implemented for the future Azure production target | 3 | 4 | **12 High** | Azure Control Coverage Gap Analysis (F-16) documents the required baseline; legacy controls cover current production; Azure not yet in production | Build and evidence the Azure security baseline (Conditional Access MFA, Key Vault soft-delete/purge/RBAC/diagnostics, Defender, Log Analytics) before cut-over; do not cut over production until F-16 controls are implemented and evidenced | Engineering Lead | Deferred | 2026-09-01 |
