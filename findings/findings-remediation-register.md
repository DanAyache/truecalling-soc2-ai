...
| **F-16** | Azure control coverage gap — Azure controls not designed/implemented for the future Azure target | Critical (at cut-over) | CC6.1/6.6/6.7, CC7.x, CC8.1, A1.2 | R-016 | Eng Lead | At Azure cut-over | **Deferred** — Azure is future-state and out of the current Type I boundary per SOC2-ENGAGEMENT-SCOPE §5/§6; risk tracked as R-016 (Deferred); reactivates at cut-over | [azure-control-coverage-gap-analysis.md](../evidence/azure/azure-control-coverage-gap-analysis.md) |
...
| **F-29** | GitHub MFA evidence contradiction — committed org members export `export-Truecalling-ai-GitHub-2026-05-27.csv` records `tfa_enabled=false` for both members (CohenYarone01, rbzil), contradicting the 2026-05-27 RBAC review's "MFA Enabled: Yes"; current live MFA state unverified | High | CC6.1 (evidence integrity) | R-002 | Eng Lead | 2026-06-20 | **Open** (raised 2026-06-14 — verify live state and re-evidence with an attributed current export; relocate the CSV into `evidence/access-control/`) | container-root `export-Truecalling-ai-GitHub-2026-05-27.csv` (pending relocation); RBAC review correction queued (T5b, awaiting approval) |
